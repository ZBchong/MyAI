...
discover_codex_app() {
  ...
  for candidate in "$configured" "/Applications/Codex.app" "$HOME/Applications/Codex.app" "/Applications/ChatGPT.app" "$HOME/Applications/ChatGPT.app"; do
...
require_macos_runtime() {
  ...
  if [ ! -x "$RUNTIME_NODE" ]; then
    RUNTIME_NODE="$CODEX_BUNDLE/Contents/Resources/node"
  fi
  [ -x "$RUNTIME_NODE" ] || fail "The signed Node.js runtime bundled with Codex was not found in the supported resource paths."
