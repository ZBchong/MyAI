...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  codex_bundle="$(shell_quote "$CODEX_BUNDLE")"
  screenshot="$(shell_quote "$HOME/Desktop/Codex Dream Skin Verification.png")"
  write_launcher "$HOME/Desktop/Codex Dream Skin.command" "CODEX_APP_BUNDLE=$codex_bundle exec $start_script --port $PORT --prompt-restart"
  write_launcher "$HOME/Desktop/Codex Dream Skin - Customize.command" "CODEX_APP_BUNDLE=$codex_bundle exec $customize_script"
  write_launcher "$HOME/Desktop/Codex Dream Skin - Verify.command" "CODEX_APP_BUNDLE=$codex_bundle $verify_script --screenshot $screenshot && /usr/bin/open $screenshot"
  write_launcher "$HOME/Desktop/Codex Dream Skin - Restore.command" "CODEX_APP_BUNDLE=$codex_bundle exec $restore_script --restore-base-theme --restart-codex"
fi
